...
async function main() {
  ...
  const enterResponse = await lockFactory.receiveETH(recipientAddress);
  await enterResponse.wait();
  console.log(enterResponse);
  let prize = await lockContract.prize;
  console.log(`The Total Prize is: ${prize}`);
